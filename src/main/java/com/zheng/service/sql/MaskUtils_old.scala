...
import com.zheng.service.yarn.SparkSqlClient.sparkSession
...
//            .master("yarn") //提交到yarn
        ...
        sql = "create table if not exists  tmp_tab as select tmp,ss from (select concat(s.the_year,c.day_of_month) tmp,s.yearly_income ss ,(s.total_children+c.unit_sales) tmp2 from default.sales_child s left join sales_child c on s.sales_region=c.sales_region) a"

        //        var sql = "insert into tab select * from default.sales_child"
        MaskUtils_old.getMaskSql(sparkSession,sql)
    }
...
//                    tableName = table.database.get + "." + tableName
//                } else {
//                    if (subquery.child.isInstanceOf[LogicalRelation] && subquery.child.asInstanceOf[LogicalRelation].relation != None &&
//                        subquery.child.asInstanceOf[LogicalRelation].relation.isInstanceOf[TiDBRelation]) {
//                        tableName = subquery.child.asInstanceOf[LogicalRelation].relation.asInstanceOf[TiDBRelation].tableRef.databaseName + "." + tableName
//                    }
//                }
//
